...
describe('#### Apple ####', function () {
    ...
    it('Can validate apple in-app-purchase w/ auto-service detection', function (done) {
        var path = process.cwd() + '/test/receipts/apple_app';
        var iap = require('../');
        iap.config({
            verbose: true
        });
    ...
    it('Can detect a valid receipt that bought nothing', function (done) {
        var path = process.cwd() + '/test/receipts/apple_no_purchases';
        var iap = require('../');
        iap.config({
            verbose: true
        });
        iap.setup(function (error) {
            ...
            fs.readFile(path, function (error, data) {
                ...
                iap.validate(iap.APPLE, receipt, function (error, response) {
                    assert.equal(error, undefined);
                    assert.equal(iap.isValidated(response), true);
                    var purchaseData = iap.getPurchaseData(response, { ignoreExpired: true });
                    assert.notEqual(purchaseData, null);
                    assert.equal(purchaseData.length, 0);
                    done();
                });
            });
        });
    ...
    it('Can validate app purchase was NOT a preorder', function (done) {
        var path = process.cwd() + '/test/receipts/apple_app';
        var iap = require('../');
        iap.config({
            verbose: true
        ...
        iap.setup(function (error) {
            ...
            fs.readFile(path, function (error, data) {
                ...
                iap.validate(iap.APPLE, receipt, function (error, response) {
                    assert.equal(error, undefined);
                    assert.equal(iap.isValidated(response), true);
                    assert.equal(response.receipt["preorder_date_ms"], undefined);
                    done();
                });
            });
        });
    ...
    it('Can validate app purchase was a preorder', function (done) {
        var path = process.cwd() + '/test/receipts/apple_app_preorder';
        var iap = require('../');
        iap.config({
            verbose: true
        });
        iap.setup(function (error) {
            ...
            fs.readFile(path, function (error, data) {
                ...
                iap.validate(iap.APPLE, receipt, function (error, response) {
                    assert.equal(error, undefined);
                    assert.equal(iap.isValidated(response), true);
                    assert.notEqual(response.receipt["preorder_date_ms"], undefined);
                    done();
                });
            });
        });
