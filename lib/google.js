...
function checkSubscriptionStatus(data, cb) {
    ...
    if (data.purchaseState !== 0) {
        verbose.log(NAME, 'Purchase is not completed:', data.purchaseState);
        return cb(new Error('PurchaseState'), createErrorData({ data: data }, {
            status: constants.VALIDATION.FAILURE,
            message: 'PurchaseState is' + data.purchaseState
        }));
    }
